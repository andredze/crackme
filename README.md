...
### **Ключевой момент**
В стеке выделяется память размером 11h = 17d байт (1 байт для флага правильности пароля и 16 байт для хранения строки с паролем), однако копируется столько, сколько фактически было записано в буфер (может быть больше 16-и байт). Изначальный размер буфера 50 байт, а выделено в стеке под строку лишь 16 байт!<br>
...
## 1-я уязвимость
Флаг правильности пароля (я назвал его access_status в дизассемблере) хранится в стеке сразу после 16 байт буфера. В конце функции этот флаг копируется из стека в регистр AL. После чего в теле программы проверяется именно регистр AL, равный этому флагу в стеке. <br>
...
С помощью расширения VSCode **Hex Editor** от Microsoft, я создал бинарный файл, содержащий 16 нулевых байт, после чего байт равный 01 (для подмены флага правильности пароля) и 0D (символ переноса каретки CR) для окончания ввода
...
Резидент будет подменять код основной программы, убирая проверку флага правильности пароля, хранящегося в регистре AL, перед выводом сообщения о предоставлении доступа.
...
Также был написан патчер (*patcher.exe*) для взлома исполняемого файла *crackme.com* с графическим интерфейсом. Программа создает взломанный файл *cracked.com* с помощью патча аналогичному патчу в резиденте-эмиттере.

Чтобы убедиться, что взламывается нужный файл, программа считает его хэш и сравнивает с хэшом правильного *crackme.com*

### Компиляция и запуск
>Для начала нужно перейти в папку patcher
## Linux
Компиляция
```bash
make
...
Патчер выполнен в стиле игры **Minecraft**. В опциях есть смена темы, меняющая фон и музыку главного меню, а также сам взлом. Для написания программы была использована библиотека SDL2 языка C.
...
<table>
<tr>
<td><img src="assets/minecraft1.png"></td>
<td><img src="assets/minecraft2.png"></td>
</tr>
</table>

<p align="center">
    <img src="assets/minecraft3.png" width="50%">
</p>

#### Экран загрузки (взлом)
------------------------------------------

<p align="center">
    <img src="assets/minecraft4.png" width="50%">
</p>

## Итоги
В результате написания Patcher-а я разобрался с библиотекой SDL2 и ознакомился с программированием приложений с графическим интерфейсом
