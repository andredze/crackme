...
Задача данного проекта заключалась в том, чтобы взломать чужой исполняемый файл, не изменяя его напрямую. А также написать свою программу с двумя уязвимостями и предоставить ее для взлома своему другу.

Для этого использовалась операционная система DOS, а именно ее эмулятор DOSBox, так как она позволяет делать низкоуровневые изменения, из-за которых в современных ОС происходит segmentation fault
...
Видно, что программа содержит в себе основную часть, и одну функцию sub_32
...
Открыв данный код в Turbo Debugger, определим, что адрес первой команды 0100h, что означает об использовании директивы org 100h, выделяющий под *Program Segment Prefix (PSP)* 100h байт

Отсюда делаем вывод, что к относительным адресам в дизассемблере нужно будет прибавить 0100h
...
По адресу 01A9 лежит длина буфера (32h = 50d байт), после чего 50 байт, заполненных нулями (сам буфер)
...
### **Ключевой момент**
Копируются 11h = 17d байт, однако изначальный размер буфера 50 байт!<br>

Это и является уязвимостью! Таким образом, мы сможем подать на ввод больше чем 17 байт и, переполнив буфер, поменять важные поля в стеке
...
С помощью расширения VSCode **Hex Editor** от Microsoft, я создал бинарный файл, содержащий 16 нулевых байт, после чего байт равный 01 (флаг проверки) и 0D (символ переноса каретки CR) для окончания ввода
...
Заменив его, мы можем передать управление в любое место программы, в том числе на вывод строки о предоставлении доступа ("Access granted"), тем самым обманув программу.
...
Учитывая **PSP** = 100h, необходимый адрес 011C. Также для записи в память используется **Little-endian** из-за чего записывать в ввод нужно будет в обратном порядке - **1C01**

Затрем сохраненное значение BP нулями, так как оно все равно не будет использовано

Поэтому нам нужно подать на ввод 19 мусорных байт, после чего записать 1С 01 - новый адрес возврата и символ переноса каретки 0D
...
NOP NOP
```
(не исполнять никакую операцию), я уберу эту проверку и программа независимо от ввода будет выводить "Access granted"
...
Моя программа не проверяла длину вводимой строки и позволяла переполнить буфер, что и являлось главной неуязвимостью.

#### Обе уязвимости заключались в переполнении буфера:
----------------

1. Поле с хэшом лежит в памяти сразу после буфера ввода, что позволяет переписать его на необходимое
