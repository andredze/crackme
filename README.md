...
Задача данного проекта заключалась в том, чтобы взломать чужой исполняемый файл, не изменяя его напрямую.
...
После этого я переписываю значение в памяти по адресу CS:[0126] и CS:[0127] на NOP-ы
