...
Задача данного проекта заключалась в том, чтобы взломать чужой исполняемый файл, не изменяя его.
...
### **Ключевой момент**
В стеке выделяется память размером 11h = 17d байт, однако копируется столько, сколько фактически было записано в буфер (может быть больше 17-и байт). Изначальный размер буфера 50 байт, а выделено в стеке лишь 17 байт!<br>

Это и является уязвимостью! Таким образом, мы сможем подать на ввод больше чем 17 байт и, переполнив место в стеке, отведенное под хранение строки, поменять важные поля в стеке
...
Необходимый адрес 011C. Также для записи в память используется **Little-endian** из-за чего записывать в ввод нужно будет в обратном порядке - **1C01**
